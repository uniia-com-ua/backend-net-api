...
public class LogActionControllerTests
{
	...
	{
		// Arrange
		var logs = new PageData<LogActionModel> { Items = new() { new() { Id = new ObjectId(), ModelId = 1, ModelName = "Test", UserId = "user1" } }, TotalCount = 1 };

		var paginationMock = _factory.Mocks.Mock<ILogPaginationService>();
		paginationMock.Setup(p => p.GetPagedListAsync(0, 10, null)).ReturnsAsync(logs);

		var client = _factory.CreateClient();

		// Act
		var response = await client.GetAsync("/api/v1/log-actions/page");
		var returned = await DeserializeResponse<PageData<LogActionModel>>(response);
	...
	public async Task GetLogByModelId_ReturnsOkWithFilteredLogs()
	{
		...
		var paginationMock = _factory.Mocks.Mock<ILogPaginationService>();
		paginationMock.Setup(p => p.GetPagedListAsync(1, "Test", 0, 10, null)).ReturnsAsync(logs);
		...
		Assert.Equal(HttpStatusCode.OK, response.StatusCode);
		Assert.Single(returned!.Items);
		Assert.All(returned!.Items, x => Assert.Equal(1, x.ModelId));
		Assert.All(returned!.Items, x => Assert.Equal("Test", x.ModelName));
	}
	...
	public async Task GetByUserId_ReturnsOkWithUserLogs()
	{
		...
		var paginationMock = _factory.Mocks.Mock<ILogPaginationService>();
		paginationMock.Setup(p => p.GetPagedListAsync("user1", 0, 10, null)).ReturnsAsync(logs);
		...
		Assert.Equal(HttpStatusCode.OK, response.StatusCode);
		Assert.Single(returned!.Items);
		Assert.All(returned!.Items, x => Assert.Equal("user1", x.UserId));
	}
